...
update_omega_exp <- function(pi_pK_rna, x_rna, v_rna, q_exp, pi_exp, pi_pK_atac, 
                             theta, phi, pi_p, overlap_seq_atac, overlap_seq_rna){
...
simData <- function(n1, n2, p, Xprobs, muG1, sigmaG1, muG2, sigmaG2,
  ...
  num_overlap <- round(p*overlap_prop)
  pi_gA_1[,(num_overlap+1):p] <- matrix(rep(q_exp, num_nonoverlap), nrow=ncol(pi_exp))
  pi_gA_0[,(num_overlap+1):p] <- 0
  v_rna <- u_rna
